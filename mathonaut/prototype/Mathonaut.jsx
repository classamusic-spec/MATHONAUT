...
export function createGame(root, T3) {
  ...
  const MAX_LEVEL = LEVELS.length;   // 13
  const levelName = (l) => LEVELS[Math.min(LEVELS.length, Math.max(1, l)) - 1];
  ...
  function tick() {
    ...
    if ((diagFrame = (diagFrame + 1) % 3) === 0) {
      ...
      window.__THREE_GAME_DIAGNOSTICS__ = {
        ...
        beamLane2: bossAtk.lane2, cargo, chain, nodes, riftGap: +(riftZ - PLAYER_Z).toFixed(1),
